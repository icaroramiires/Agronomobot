    ...
    this.conversation.message({
      ...
      if (err) {
        ...
        if (inputParser.isAskingForWelcome(resp)) {
          return handlers.command.sendMessageWelcome(message, resp.output.text[0], this.bot)
        }

        if (inputParser.isAskingForMenu(resp)) {
          return handlers.command.sendMessageMenu(message, resp.output.text[0], this.bot)
        }

        if (inputParser.isAskingForHelp(resp)) {
          return handlers.command.sendMessageHelp(message, resp.output.text[0], this.bot)
        }

        if (inputParser.isAskingForInfoNodes(resp)) {
          return handlers.command.sendInfoNodes(message, resp.output.text[0], this.bot)
        }
      }
    })
  }
